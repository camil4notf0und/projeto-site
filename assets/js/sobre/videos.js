...
[...videos].forEach((video) => {
  ...
  video.addEventListener("click", function() {
    if(tocando == false) {
      ...
      tocando = true;
      maximixaMinimiza = 0;
      let tituloOffCanvas = document.getElementById("offcanvas-title-segundo-plano");
      let tituloPodcast = this.title;

      
      if(tituloPodcast.length <= 28) {
        tituloOffCanvas.textContent = tituloPodcast;
      }else {
        let quebrado = tituloPodcast.split("");
        ...
        tituloOffCanvas.title = tituloPodcast;
        tituloOffCanvas.textContent = juntado;
      }
...
function segundoPlano() {
    ...
    if(tocando == false) {
          for (x = 0; x < videos.length; x++) {
              videos[x].setAttribute("data-bs-toggle", "offcanvas");
    ...
    if(tocando == true) {
      ...
      videos[qualTocando].setAttribute("data-bs-toggle", "offcanvas");
